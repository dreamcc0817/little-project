...
import com.dreamcc.little.project.rocketmq.api.coupon.service.CouponService;
import com.dreamcc.little.project.rocketmq.api.hotel.dto.HotelRoom;
import com.dreamcc.little.project.rocketmq.api.hotel.service.HotelRoomService;
...
import com.dreamcc.little.project.rocketmq.api.order.dto.OrderItemDTO;
import com.dreamcc.little.project.rocketmq.api.order.enums.OrderBusinessErrorCodeEnum;
import com.dreamcc.little.project.rocketmq.api.order.enums.OrderStatusEnum;
...
import com.dreamcc.little.project.rocketmq.common.constants.StringPoolConstant;
import com.dreamcc.little.project.rocketmq.common.exception.BusinessException;
...
public class OrderServiceImpl implements OrderService {
    ...
    private void saveOrderItemInfo(OrderInfoDTO orderInfoDTO) {
        ...
        // 保存订单商品
        log.info("start save orderItem param:{}", JSON.toJSONString(mysqlRequestDTO));
        CommonResponse<Integer> commonResponse = mysqlApi.insert(mysqlRequestDTO);
        log.info("end save orderItem param:{}, response:{}", JSON.toJSONString(mysqlRequestDTO), JSON.toJSONString(commonResponse));
    ...
    private void saveOrderInfo(OrderInfoDTO orderInfoDTO) {
        ...
        log.info("start save order param:{}", JSON.toJSONString(mysqlRequestDTO));
        CommonResponse<Integer> insertOrderResponse = mysqlApi.insert(mysqlRequestDTO);
        log.info("end save order param:{}, response:{}", JSON.toJSONString(mysqlRequestDTO), JSON.toJSONString(insertOrderResponse));
        if (Objects.equals(ErrorCodeEnum.SUCCESS.getCode(), insertOrderResponse.getCode())) {
            ...
            // 订单保存失败
            log.error("save order fail error message:{}", JSON.toJSONString(insertOrderResponse));
